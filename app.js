...
document.getElementById('btnImportHSK1').addEventListener('click', ()=>{
  openModal(`
    ...
    <p style="font-size:12px;color:var(--text-muted);margin-top:10px;">Chữ sẽ tự xếp theo <b>số nét từ ít đến nhiều</b> (dễ trước, khó sau) — không xếp lộn xộn như trước nữa. Chữ đã có trong kho sẽ được bỏ qua.</p>
    <div class="modal-actions">
      <button class="btn btn-ghost" data-close>Huỷ</button>
  ...
  document.getElementById('mImportConfirm').addEventListener('click', async ()=>{
    ...
    const sorted = [...toAdd].sort((a,b)=> (counts[a]||99) - (counts[b]||99));
